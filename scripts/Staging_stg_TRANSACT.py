...
item_DEPORT = item_DEPORT.rename(columns={
    'DEP_TS' : 'EXEC_TS'
})
...
unique_year = set([row['EXEC_TS'].to_pydatetime().year for i, row in stg_transact.iterrows()])
sample = []
...
for i in range(len(f_transact)):
    ...
    f_transact.loc[i, 'EXEC_TS'] = sample['DATE_ID'].tolist()[0]
...
dwh_engine.execute('alter table d_DATE alter column DATE_ID char(6) not null')
...
dwh_engine.execute('alter table f_TRANSACT add constraint FK_DATEID_D_DATE foreign key (EXEC_TS) references d_DATE(DATE_ID)')
